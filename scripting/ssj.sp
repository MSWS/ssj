...
void ShowSSJMenu(int client, int position = 0) {
    ...
    if (g_bEnabled[client])
        ...
        AddMenuItem(menu, "usage", "Usage: [OFF]");
...
void SSJ_PrintStats(int client, int target) {
    ...
    if (distance > g_flTrajectory[target])
        distance = g_flTrajectory[target];
    if (distance > 0.0)
